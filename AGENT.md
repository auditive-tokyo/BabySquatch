...
- **Click HPF/LPF のフィルター有効/無効判定の見直し**（要方針決定）
  - **現状の問題**: `flags.hpf = hpfQv > 0.001f` — Reso(Q)=0 のとき HPF/LPF が完全バイパスされる
  - **影響**: Tone/Noise モードでは Reso デフォルト=0 のため、Freq ノブ・Slope セレクターが一切効かない。完全に無意味なコントロールになっている
  - **Sample モードとの不整合**: Sample モードは Q=0.1〜6.0 の range なので Q=0 になれず常にフィルターが効く。「オフ」は Freq=20Hz / 20000Hz で表現
  - **決めるべき事項**:
    1. フィルターのオン/オフを「Q=0 でバイパス」にするか「Freq の端値（20Hz/20kHz）でバイパス」に統一するか
    2. Q=0 時に内部で Butterworth（Q=0.707）に固定するか、それとも別途「フィルターオフ」UI を設けるか
    3. Tone/Noise の Reso ノブ range を 0〜12 のまま維持するか、0.707 固定下限にするか
  - **修正候補（統一案）**: `flags.hpf = hpfStg > 0` に変更し、Q=0 時は 0.707 固定。Tone/Noise でも Freq・Slope が常に有効になる

- **Tone モードの Decay パラメーターの扱い**（要方針決定）
  - **現状の問題**: Tone モードでは `amp = exp(-t × 5000 / decayMs)` を掛けているが、BPF の自然減衰（数十〜数百μs）が先に支配するため Decay の効果がほぼ聴覚的に無効
  - **計算**: 5kHz/Q=0.71 では -60dB 到達が ~0.3ms（Decay=2000ms でも）。Q=12 + 低周波でも最大 2-3ms
  - **選択肢**:
    1. **現状維持** — Decay は Tone では実質無効だが UI に残す（細かい違いは存在するため）
    2. **Tone では Decay 非表示** — Noise のみ Decay スライダーを表示する
    3. **DSP 変更** — Tone モードの Decay エンベロープを撤去し最後まで鳴らす（合わせて maxTimeSamples も変更）
