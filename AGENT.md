...
- **Click HPF/LPF のフィルター有効/無効判定の見直し**（要方針決定）
  ...
  - **影響**: Noise モードでは Reso デフォルト=0 のため、Freq ノブ・Slope セレクターが一切効かない。完全に無意味なコントロールになっている
  ...
  - **決めるべき事項**:
    ...
    3. Noise の Reso ノブ range を 0〜12 のまま維持するか、0.707 固定下限にするか
  - **修正候補（統一案）**: `flags.hpf = hpfStg > 0` に変更し、Q=0 時は 0.707 固定。Noise でも Freq・Slope が常に有効になる
